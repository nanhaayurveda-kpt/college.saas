...
export default function AttendanceForm({
  ...
  function markAllNA() {
    ...
    if (!form) return;
    form.querySelectorAll('input[type="radio"][value="na"]').forEach((r) => {
      r.checked = true;
    });
  }

  return (
    <form ref={formRef} method="POST" action="/api/attendance/save"
      ...
      {sortedKeys.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-100 px-4 py-2 mb-3 flex items-center">
          ...
          <div className="flex items-center gap-3 shrink-0 text-xs font-bold">
            ...
            <button type="button" onClick={markAllNA}
      ...
      {sortedKeys.length > 0 && (
        <button type="submit" disabled={submitting}
          className="w-full bg-indigo-600 text-white py-3 rounded-xl text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed">
