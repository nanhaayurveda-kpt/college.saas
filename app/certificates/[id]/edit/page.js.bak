...
export default async function EditCertificatePage({ params }) {
  ...
  const rows = await db
    ...
    .where(and(eq(certificates.id, certId), eq(certificates.user_id, 1)));
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 max-w-2xl">
        <form action="/api/certificates/update" method="POST" className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="date" name="issue_date" defaultValue={cert.issue_date || ""} required
                className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Serial No</label>
              <input type="text" name="serial_no" defaultValue={cert.serial_no || ""}
                className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
          </div>
          ...
          <div>
            ...
            <select name="conduct" defaultValue={cert.conduct || "Good"}
              className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Last Course Attended</label>
            <input type="text" name="last_course" defaultValue={cert.last_course || ""}
              className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Last Exam Passed</label>
            <input type="text" name="last_exam_passed" defaultValue={cert.last_exam_passed || ""}
              className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Reason / Purpose</label>
            <input type="text" name="reason" defaultValue={cert.reason || ""}
              className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Additional Notes</label>
            <textarea name="custom_content" rows={3} defaultValue={cert.custom_content || ""}
