    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-100 p-10 text-center text-gray-400 text-sm">
          No exams found. <Link href="/exams/add" className="text-indigo-600 font-medium">Schedule one →</Link>
        </div>
      ) : (
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      ...
                      <span className={`px-2 py-0.5 text-xs rounded-full font-medium ${isUpcoming ? "bg-indigo-100 text-indigo-700" : "bg-gray-100 text-gray-500"}`}>
                        {isUpcoming ? "Upcoming" : "Completed"}
                      </span>
                    </div>
                    <p className="text-xs text-gray-500">
                      {exam.course} {exam.semester ? `Sem ${exam.semester}` : ""} · {exam.subject}
                    </p>
                    ...
                    {hasResults && (
                      <p className="text-xs text-green-600 mt-0.5">✓ {exam.result_count} results entered</p>
                    )}
                  </div>
