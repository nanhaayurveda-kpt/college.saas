...
export async function POST(request, { params }) {
  ...
  if (!packageId) return NextResponse.redirect(new URL("/fee-structure", request.url), { status: 303 });
  ...
  for (const feeType of FIXED_TYPES) {
    ...
    items.push({ fee_type: feeType, amount: amt, label: null });
  }
  ...
  if (dupRows.length > 0) {
    await setFlash("error", `Another package for ${course} (${academic_year}) already exists.`);
  ...
  await setFlash("success", `Package updated — ${course}${semester ? ` ${semester}` : ""} (${academic_year}), ₹${computedTotal}`);
  return NextResponse.redirect(new URL("/fee-structure", request.url), { status: 303 });
}