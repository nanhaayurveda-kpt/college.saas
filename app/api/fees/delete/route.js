...
export async function POST(request) {
  ...
  if (isNaN(id)) return NextResponse.redirect(new URL("/fees", request.url), { status: 303 });

  const ownRows = await db.select({ id: schema.fees.id })
    .from(schema.fees)
    .where(and(eq(schema.fees.id, id), eq(schema.fees.user_id, 1)));

  if (!ownRows.length) return NextResponse.redirect(new URL("/fees", request.url), { status: 303 });

  await db.delete(schema.fee_payments)
    .where(and(eq(schema.fee_payments.fee_id, id), eq(schema.fee_payments.user_id, 1)));
