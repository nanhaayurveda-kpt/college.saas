...
export async function POST(request, { params }) {
  ...
  if (!feeId) return NextResponse.redirect(new URL("/fees", request.url), { status: 303 });
  ...
  const fee_type = formData.get("fee_type") || "semester";
  ...
  if (!due_date_raw || isNaN(amount) || amount <= 0) {
    ...
    return NextResponse.redirect(new URL(`/fees/${feeId}/edit`, request.url), { status: 303 });
  }
  ...
  else if (status === "partial") paid_amount = parseInt(formData.get("paid_amount"), 10) || 0;
