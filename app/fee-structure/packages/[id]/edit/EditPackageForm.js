...
export default function EditPackageForm({ pkg }) {
  ...
  for (const it of pkg.items) {
    if (FIXED_VALUES.has(it.fee_type)) {
      ...
      initialCustom.push({
        name: it.label || humanize(it.fee_type),
        amount: String(it.amount),
      });
    }
  }

  const [selectedCourse, setSelectedCourse] = useState(pkg.course || "");
  const [selectedSemester, setSelectedSemester] = useState(pkg.semester || "");
  ...
  const total = useMemo(() => {
    ...
    const custom = customItems.reduce(
      (sum, it) => sum + (parseInt(it.amount) || 0), 0,
    );
    return fixed + custom;
  ...
  return (
    <form method="POST" action={`/api/fee-structure/packages/${pkg.id}/edit`}
      ...
      <div>
        ...
        <input type="text" name="academic_year" required value={academicYear}
      ...
      <div>
        <div className="flex justify-between items-center mb-2">
          ...
          <button type="button" onClick={addCustomItem}
            className="text-xs text-indigo-600 font-medium">+ Add Item</button>
        </div>
