...
import DeleteProfessor from "./DeleteTeacher";
...
const courses = [
  "B.A.", "M.A.", "B.Com", "M.Com", "B.Sc.", "M.Sc.", "B.Sc. Ag.", "M.Sc. Ag.",
];
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <Link href="/professors" className="bg-gray-100 text-gray-700 px-4 py-2 rounded-lg text-sm font-medium">
          ← Back
        </Link>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-4 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-indigo-600 font-medium">{designationLabel[p.designation] || p.designation}</p>
          {p.qualification && <p className="text-xs text-gray-500 mt-0.5">{p.qualification}</p>}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-gray-900">
              {p.joining_date ? new Date(Number(p.joining_date)).toLocaleDateString("en-IN") : "—"}
            </p>
          </div>
      ...
      <div className="flex gap-3 mb-4">
        <Link href={`/professors/${p.id}/edit`}
          className="flex-1 text-center bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-medium">
          ✏️ Edit Professor
        </Link>
        <DeleteProfessor professorId={p.id} professorName={p.name} />
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        ...
        <form method="POST" action="/api/professors/add-subject" className="space-y-3">
          ...
          <div>
            ...
            <input type="text" name="subject" required placeholder="e.g. Economics, Hindi, Computer Science"
