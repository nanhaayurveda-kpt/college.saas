...
export default async function AdmissionsPage() {
  ...
  newStudents.forEach((s) => {
    ...
    if (!newByCourse[course]) newByCourse[course] = [];
    newByCourse[course].push(s);
  });

  const oldByCourse = {};
  oldStudents.forEach((s) => {
    const course = s.course || "—";
    oldByCourse[course] = (oldByCourse[course] || 0) + 1;
  });
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        ...
        <div className="bg-yellow-50 rounded-xl p-3 border border-yellow-100 text-center">
          ...
          <div className="text-xs text-yellow-600 mt-1">Returning</div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-5">
        ...
        <div className="divide-y divide-gray-50">
          {allCourses.map((course) => {
            const nw = newByCourse[course]?.length || 0;
            const old = oldByCourse[course] || 0;
            return (
              <div
                key={course}
      ...
      {newStudents.length > 0 && (
        <div className="mb-5">
          ...
          <div className="space-y-4">
            {allCourses
              .filter((c) => newByCourse[c]?.length > 0)
              .map((course) => (
                <div
                  key={course}
                ...
                >
                  <div className="bg-green-50 px-4 py-2.5 flex justify-between items-center border-b border-green-100">
                    ...
                    <span className="text-green-600 text-xs">
                      {newByCourse[course].length} new admissions
                    </span>
                  </div>
                  <div className="divide-y divide-gray-50">
                    {newByCourse[course].map((s, idx) => (
                      <div
                        key={s.id}
